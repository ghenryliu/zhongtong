<template>
  <div class="mainpart">
    ...
    <div class="content">
      <div>
        <p><span>账号</span><span>{{name}}</span></p>
        <p><span>身份证号</span><span>{{id}}</span></p>
  ...
  export default {
    ...
    methods: {
      submit() {
        ...
        this.$router.push({'path': '/approveMaintainDetail'})
      },
      showFillReason() {
        this.isDisplay = true
...
<style>
  * {
  ...
  .content div:first-child > p > span:first-child {
    width: 5vw;
    height: 2rem;
    margin-left: 2.7rem;
    margin-right: 1.1rem;
  ...
  .content div:first-child > p > span:last-child {
    font-weight: bold;
    font-family: PingFangSC-Regular;
    font-size: 0.8rem;
    color: #C6CBD4;
  ...
  .content div:last-child > span:last-child {
    width: 7.5rem;
    height: 2rem;
    background: #fff;
    border-radius: 24px;
  ...
  .notapprove {
    ...
    flex-direction: column;
    align-items: center;
    box-shadow: 0 0 80px 35px rgba(0, 0, 0, .1);
  }
  ...
  .notapprove > div > span:last-child {
    font-family: PingFangSC-Regular;
    font-size: 0.7rem;
    color: #4B74FF;
    letter-spacing: 0;
